...
int main(int argc, char **argv)
{
    ...
    for (size_iter= 0; size_iter < STEP; size_iter= size_iter + 1) {
...
#endif
        //touch data;   avoid cache cold start
        ...
        if (rank == 0)
        {
            for (iteration = 0;iteration<IT; iteration++){
                ...
                MPI_Irecv(rbuffer, 1, type_cont, 1, 123, MPI_COMM_WORLD, &recvreq);
                MPI_Isend(sbuffer, 1, type_cont, 1, 234, MPI_COMM_WORLD, &sendreq);
                MPI_Wait(&sendreq, &reqstat);
                MPI_Wait(&recvreq, &reqstat);
            }
            te = MPI_Wtime();
        }
        ...
        {
            for (iteration = 0;iteration<IT; iteration++){
            MPI_Irecv(rbuffer, 1, type_cont, 0, 234, MPI_COMM_WORLD, &recvreq);
            MPI_Isend(sbuffer, 1, type_cont, 0, 123, MPI_COMM_WORLD, &sendreq);
            MPI_Wait(&sendreq, &reqstat);
            MPI_Wait(&recvreq, &reqstat);
    ...
    if (rank == 0)
    {
        ...
        if (MPI_IMP == "OPEN MPI")
			printf("MPI IMPLEMENTATION, NON/BLK, ITERATION,SIZE,DATA_TYPE,TIME FOR SENDER,BANDWIDTH(MB/s), L2 D CACHE MISS, L3 TOTAL CACHE MISS, D TLB MISS \n");
        for (size_iter = 0; size_iter < STEP; size_iter=size_iter+1){
            //if (size_iter%2 == 0){
                psize = array_size * (size_iter+1);
                hard_size = psize*psize*(sizeof(int))/4000; //kbytes
                hard_size_buk = (1+psize)*psize*(sizeof(int))/2000; //kbytes
                printf(MPI_IMP",BLK, %d,%d,Contiguous,%.6f, %d, %d, %d, %d\n", IT,psize, t1[size_iter], (int)(hard_size*40/t1[size_iter]), v1[size_iter][1],v1[size_iter][2],v1[size_iter][3]);
                printf(MPI_IMP",BLK, %d,%d,EQL_Contiguous_NOM,%.6f, %d, %d, %d, %d\n", IT,psize, t6[size_iter], (int)(hard_size*40/t6[size_iter]), v1[size_iter][1],v1[size_iter][2],v1[size_iter][3]);
                printf(MPI_IMP",BLK, %d,%d,Tiled,%.6f, %d, %d, %d, %d\n", IT,psize, t2[size_iter], (int)(hard_size*10/t2[size_iter]), v2[size_iter][1],v2[size_iter][2],v2[size_iter][3]);
                printf(MPI_IMP",BLK, %d,%d,EQL_Tiled_NOM,%.6f, %d, %d, %d, %d\n", IT,psize, t5[size_iter], (int)(hard_size*10/t5[size_iter]), v5[size_iter][1],v5[size_iter][2],v5[size_iter][3]);
                //printf(MPI_IMP",BLK, %d,%d,STRUCT_S,%.6f\n", IT,array_size*(DEPTH*size_iter+1), t3[size_iter]);
                printf(MPI_IMP",BLK, %d,%d,Bucket,%.6f, %d, %d, %d, %d\n", IT,psize, t3[size_iter], (int)(hard_size_buk*10/t3[size_iter]), v3[size_iter][1],v3[size_iter][2],v3[size_iter][3]);
                printf(MPI_IMP",BLK, %d,%d,EQL_Bucket_NOM,%.6f, %d, %d, %d, %d\n", IT,psize, t7[size_iter], (int)(hard_size_buk*10/t7[size_iter]), v4[size_iter][1],v4[size_iter][2],v4[size_iter][3]);
