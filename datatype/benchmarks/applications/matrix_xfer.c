...
int main(int argc, char **argv)
{
    ...
    MPI_Datatype type_v1, type_v2, type_v_col2, type_v_col1, type_struct, type_cont, type_cont2, type_cont_cont1, type_cont_cont2, type_resize, type_resize2, type_indexed1, type_indexed2, type_indexed_blk;
    MPI_Status status;
    ...
    for (size_iter= 0; size_iter < STEP; size_iter= size_iter + 1) {
        ...
        iteration = IT;
		int i,j,p;
		DATA1 *smatrix = (DATA1 *)malloc(total*2 * sizeof(DATA1));
		DATA1 *rmatrix = (DATA1 *)malloc(total*2 * sizeof(DATA1));
    	// Note that arr[i][j] is same as *(*(arr+i)+j)
		for (p = 0; p <  total; p++){
		     		smatrix[p] = (DATA1)rand();  // OR *(*(arr+i)+j) = ++count
					rmatrix[p] = (DATA1)0;
		}
                DATA2 *smatrix2 = (DATA2*) (&(smatrix[total]));
                DATA2 *rmatrix2 = (DATA2*) (&(rmatrix[total]));

		for (p = 0; p <  total; p++){
		     		smatrix2[p] = (DATA2)rand();  // OR *(*(arr+i)+j) = ++count
					rmatrix2[p] = (DATA2)0;
		}
        ...
        MPI_Type_vector(msize*msize, X,msize, MPI_DATA2,  &type_v_col2);  // (int *  )x n
		...
		*/
			...
			for (p = 0; p <  msize; p++){
				for (i = 0; i <  msize; i++){
                                        if (out ==0) {
                                            ...
                                            iter_disp0 = i*msize + p*msize*msize;
                                            it_disp = i + p*msize + msize*msize;
                                            if(out == 0)
                                                    MPI_Get_address(&(smatrix2[iter_disp0]), disp0+(it_disp));
                                            else
                                                    MPI_Get_address(&(smatrix2[iter_disp0]), disp0+(it_disp));

                                        }
					    disp0[it_disp] =  disp0[it_disp] - base;
                                            //printf("disp0[%d] is %08x\n", iter_diso);
				}
			}
		}
        ...
        for (iter_disp0 = 0; iter_disp0 < (blockcount0/2); iter_disp0++){
			type[iter_disp0] = MPI_DATA1;
		}
		for (iter_disp0 = (blockcount0/2); iter_disp0 < blockcount0; iter_disp0++){
			type[iter_disp0] = MPI_DATA2;
		}
...
if (DEBUG) if (rank == 0) printf("Created struct3\n");
		...
		MPI_Type_indexed(blockcount, blocklen, disp, MPI_DATA2, &type_indexed2);
        MPI_Type_commit(&type_indexed2);
		if (DEBUG) if (rank == 0) printf("Created indexed\n");

		//*****type 5: contiguous type
		MPI_Type_contiguous(X, MPI_DATA1, &type_cont);   // int int int
        MPI_Type_commit(&type_cont);

		MPI_Type_create_resized(type_cont, 0, msize * sizeof(DATA1), &type_resize );
    	MPI_Type_commit( &type_resize );

		MPI_Type_contiguous(msize*msize, type_resize, &type_cont_cont1);   // int int int
        MPI_Type_commit(&type_cont_cont1);

		MPI_Type_contiguous(X, MPI_DATA2, &type_cont2);   // int int int
        MPI_Type_commit(&type_cont2);

		MPI_Type_create_resized(type_cont2, 0, msize * sizeof(DATA2), &type_resize2 );
    	MPI_Type_commit( &type_resize2 );

		MPI_Type_contiguous(msize*msize, type_resize2, &type_cont_cont2);   // int int int
        MPI_Type_commit(&type_cont_cont2);
...
*/
		...
		DATA2 send_buffer2[msize*msize*X];
		DATA2 recv_buffer2[msize*msize*X];
		count = msize*msize*X;
            ...
            for (iteration = 0;iteration<IT; iteration++){
				...
				for (pack_iter=0; pack_iter<msize*msize; ++pack_iter){
					for (inner_iter = 0; inner_iter < X; ++inner_iter){
						...
						send_buffer2[pack_iter*X+inner_iter] = smatrix2[pack_iter*msize+inner_iter];
					}

				}

                MPI_Send(send_buffer, count, MPI_DATA1, 1, 234, MPI_COMM_WORLD);
				MPI_Send(send_buffer2, count, MPI_DATA2, 1, 235, MPI_COMM_WORLD);
				MPI_Recv(recv_buffer, count, MPI_DATA1, 1, 123, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
				MPI_Recv(recv_buffer2, count, MPI_DATA2, 1, 124, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
				for (pack_iter=0; pack_iter<msize*msize; ++pack_iter){
					for (inner_iter = 0; inner_iter < X; ++inner_iter){
						rmatrix[pack_iter*msize+inner_iter] = recv_buffer[pack_iter*X+inner_iter];
						rmatrix2[pack_iter*msize+inner_iter] = recv_buffer2[pack_iter*X+inner_iter];
					}
        ...
        if (rank == 1)
        {
            for (iteration = 0;iteration<IT; iteration++){
                ...
                MPI_Recv(recv_buffer2, count, MPI_DATA2, 0, 235, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
				for (pack_iter=0; pack_iter<msize*msize; ++pack_iter){
					for (inner_iter = 0; inner_iter < X; ++inner_iter){
						rmatrix[pack_iter*msize+inner_iter] = recv_buffer[pack_iter*X+inner_iter];
						rmatrix2[pack_iter*msize+inner_iter] = recv_buffer2[pack_iter*X+inner_iter];
                ...
                MPI_Send(send_buffer2, count, MPI_DATA2, 0, 124, MPI_COMM_WORLD);
            }
        }
		...
		for (v=0; v < msize; v++){
			for (z=0; z < msize; z++){
				...
				if ((smatrix[y] != rmatrix[y]) || (smatrix2[y] != rmatrix2[y]))
				{
					if (smatrix[y] != rmatrix[y])
						printf("y is :  %d, manually copy data1 %lf %lf wrong\n", y, smatrix[y], rmatrix[y]);
					else printf("y is :  %d, manually copy data2 %f %f wrong\n", y, smatrix2[y], rmatrix2[y]);
					exit(1);
				}
			}
		}}
        ...
        if (rank == 0)
        {
            for (iteration = 0;iteration<IT; iteration++){
                ...
                MPI_Send(smatrix2, 1, type_v_col2, 1, 235, MPI_COMM_WORLD);
                MPI_Recv(rmatrix, 1, type_v_col1, 1, 123, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(rmatrix2, 1, type_v_col2, 1, 124, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
            te = MPI_Wtime();
        }
        ...
        if (rank == 1)
        {
            ...
            MPI_Recv(rmatrix2, 1, type_v_col2, 0, 235, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Send(smatrix, 1, type_v_col1, 0, 123, MPI_COMM_WORLD);
            MPI_Send(smatrix2, 1, type_v_col2, 0, 124, MPI_COMM_WORLD);
		...
		for (v=0; v < msize; v++){
			for (z=0; z < msize; z++){
				...
				if ((smatrix[y] != rmatrix[y]) || (smatrix2[y] != rmatrix2[y]))
				{
					...
					else printf("v is :  %d, vector data2 %f %f wrong\n", v, smatrix2[y], rmatrix2[y]);
					exit(1);
				}
			}
		}}
		...
		for (v=0; v < msize; v++){
			for (z=0; z < msize; z++){
				...
				if ((smatrix[y] != rmatrix[y]) || (smatrix2[y] != rmatrix2[y]))
				{
					...
					else printf("v is :  %d, struct copy data2 %f %f wrong\n", v, smatrix2[y], rmatrix2[y]);
					exit(1);
				}
			}
		}}
        ...
        if (rank == 0)
        {
            for (iteration = 0;iteration<IT; iteration++){
                if(iteration == 0) ts = MPI_Wtime();
                MPI_Send(smatrix, 1, type_cont_cont1, 1, 234, MPI_COMM_WORLD);
                MPI_Send(smatrix2, 1, type_cont_cont2, 1, 235, MPI_COMM_WORLD);
                MPI_Recv(rmatrix, 1, type_cont_cont1, 1, 123, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(rmatrix2, 1, type_cont_cont2, 1, 124, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
            te = MPI_Wtime();
        }
        ...
        {
            for (iteration = 0;iteration<IT; iteration++){
                MPI_Recv(smatrix, 1, type_cont_cont1, 0, 234, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(smatrix2, 1, type_cont_cont2, 0, 235, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Send(rmatrix, 1, type_cont_cont1, 0, 123, MPI_COMM_WORLD);
                MPI_Send(rmatrix2, 1, type_cont_cont2, 0, 124, MPI_COMM_WORLD);
            }
        }
		...
		for (v=0; v < msize; v++){
			for (z=0; z < msize; z++){
				...
				if ((smatrix[y] != rmatrix[y]) || (smatrix2[y] != rmatrix2[y]))
				{
					...
					else printf("v is :  %d, indexed data2 %f %f wrong\n", v, smatrix2[y], rmatrix2[y]);
					exit(1);
				}
			}
		}
        ...
        if (rank == 0)
        {
            for (iteration = 0;iteration<IT; iteration++){
                ...
                MPI_Send(smatrix2, 1, type_indexed2, 1, 235, MPI_COMM_WORLD);
                MPI_Recv(rmatrix, 1, type_indexed1, 1, 123, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Recv(rmatrix2, 1, type_indexed2, 1, 124, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
            te = MPI_Wtime();
        }
        ...
        if (rank == 1)
        {
            for (iteration = 0;iteration<IT; iteration++){
                ...
                MPI_Recv(smatrix2, 1, type_indexed2, 0, 235, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                MPI_Send(rmatrix, 1, type_indexed1, 0, 123, MPI_COMM_WORLD);
                MPI_Send(rmatrix2, 1, type_indexed2, 0, 124, MPI_COMM_WORLD);
            }
        }
